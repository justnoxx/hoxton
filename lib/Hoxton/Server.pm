...
sub start_listen {
    ...
    for my $listen (@listen) {
        push @{$self->{listen_guards}}, $self->_create_tcp_server($listen, $app)->run();
    }
}
...
sub _create_tcp_server {
    ...
    if ($self->{port}) {
        $server_options->{port} = $self->{port};
    }

    if ($self->{daemonize}) {
        $server_options->{daemonize} = 1;
    }

    if ($self->{workers}) {
        $server_options->{workers} = $self->{workers};
    }

    if ($self->{debug}) {
        $server_options->{debug} = 1;
    }

    if ($self->{pid}) {
        $server_options->{pid} = $self->{pid};
    }

    $server_options->{procname} = 'Hoxton';
    $server_options->{port} ||= 3000;
    $server_options->{workers} ||= 5;

    $server_options->{client_forwarding} = 1;

    $server_options->{process_request} = sub {
        $self->{exit_guard} = AnyEvent->condvar();
        $self->_accept_handler($app, $is_tcp, \$listen_host, \$listen_port)->(@_);
    };

    my $aesrvr = AETCPSRVR->new(%$server_options);
    return $aesrvr;
}
        ...
        $res->(
            sub {
                ...
                if ( @$res < 2 ) {
                    ...
                    my $writer = Hoxton::Writer->new($sock, $self->{exit_guard});
